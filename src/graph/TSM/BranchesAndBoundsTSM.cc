...
TravelingSalesman::TsmResult BranchesAndBoundsTSM::solve(const Graph &graph) {
  ...
  while (!candidate_queue.empty()) {
    ...
    if (current_node.current_path.size() == graph.size()) {
      ...
      while (!candidate_queue.empty() &&
             candidate_queue.top().calculated_cost > problem_upper_bound) {
        candidate_queue.pop();
      }
    }
    ...
    for (auto way : possible_ways) {
      ...
      makeInfinitiesInMatrix(next_matrix, current_node.city_number - 1, way - 1,
                             current_node.current_path.front() - 1);
      auto reduced = reduceMatrix(next_matrix);

      next.calculated_cost =
          current_node
              .calculated_matrix[current_node.city_number - 1][way - 1] +
          reduced.first + current_node.calculated_cost;
...
std::vector<int> BranchesAndBoundsTSM::getPossibleWays(const Graph &graph,
  ...
  for (int i : neighbors) {
    if (std::find(node.current_path.begin(), node.current_path.end(), i) ==
...
void BranchesAndBoundsTSM::makeInfinitiesInMatrix(Matrix &m, int from, int to,
                                                  int first_city) {
  for (size_t i = 0; i < m.size(); i++) {
