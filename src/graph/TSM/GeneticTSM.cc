...
void GeneticTSM::initPopulation(std::vector<TsmResult> &population, const Graph& graph) {
...
void GeneticTSM::calculateFitness(TsmResult &individ, const Graph& graph) {
    ...
    for (size_t i = 0; i < individ.vertices.size() - 1; i++) {
        individ.distance += graph.getEdgeCost(individ.vertices[i], individ.vertices[i + 1]);
    }

    individ.distance += graph.getEdgeCost(individ.vertices.back(), individ.vertices.front());
}
