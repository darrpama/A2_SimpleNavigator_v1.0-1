...
void AntColonyTSM::updatePheromones(const Graph &graph, Matrix &pheromones,
                                    const std::vector<int> &visited,
                                    double way_length) {
  for (size_t i = 0; i < visited.size() - 1; ++i) {
    pheromones[visited[i] - 1][visited[i + 1] - 1] +=
        constant::Q / graph.size() / way_length;
    pheromones[visited[i + 1] - 1][visited[i] - 1] +=
        constant::Q / graph.size() / way_length;
  }
}

TravelingSalesman::TsmResult AntColonyTSM::runAnts(const Graph &graph,
                                                   Matrix &pheromones) {
  TsmResult result;
  ...
  for (size_t ant = 0; ant < graph.size(); ++ant) {
    ...
    while (possible_ways.size() != 0) {
      current_pos =
          selectNextWay(graph, pheromones, current_pos, possible_ways);
...
size_t AntColonyTSM::selectNextWay(const Graph &graph, Matrix &pheromones,
                                   size_t current_pos,
                                   const std::vector<int> &possible_ways) {
  double sum = 0;
  for (auto way : possible_ways) {
    sum +=
        std::pow(pheromones[current_pos][way - 1], constant::ALPHA) *
  ...
  for (size_t i = 0; i < possible_ways.size(); ++i) {
    probabilities[i] =
        std::pow(pheromones[current_pos][possible_ways[i] - 1],
                 constant::ALPHA) *
        std::pow(1.0 / graph.getEdgeCost(current_pos + 1, possible_ways[i]),
                 constant::BETA) /
        sum;
  }
...
size_t AntColonyTSM::getProbabilityMatch(
...
void AntColonyTSM::filterWays(std::vector<int> &possible_ways,
                              const std::vector<int> &visited) {
  size_t erased = 0;
