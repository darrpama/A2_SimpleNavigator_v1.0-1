...
void ConsoleInterface::helloMsg() const {
  system("clear");
  std::cout << "Hello, this is a console interface of SimpleNavigator."
            << std::endl;
  std::cout << "The project about few graph algorithms." << std::endl;
  std::cout << "To use this program load graph from adjacency matrix and enter "
               "any number from menu."
            << std::endl;
...
void ConsoleInterface::menuMsg() {
  ...
  std::cout << "5. Dijkstra's algorithm for finding shortest path."
            << std::endl;
  std::cout << "6. Floyd-Warshall algorithm for finding shortest paths between "
               "all combinations."
            << std::endl;
  std::cout << "7. Prim's algorithm for finding least spanning tree of graph."
            << std::endl;
  std::cout
      << "8. Ant-Colony algorithm for solving the Traveling Salesman Problem."
      << std::endl;
  std::cout << "9. Branches And Bounds algorithm for solving the Traveling "
               "Salesman Problem."
            << std::endl;
  std::cout
      << "10. Genetic algorithm for solving the Traveling Salesman Problem."
      << std::endl;
  std::cout
      << "11. Research between Traveling Salesman Problem solving algorithms."
      << std::endl;

  std::cout << "-1. Exit." << std::endl;

  std::cout << "Please select an option." << std::endl;
  while ((option_ < InterfaceOption::LOAD_GRAPH ||
          option_ > InterfaceOption::TSM_RESEARCH) &&
         option_ != InterfaceOption::EXIT) {
    int tmp_opt;
    std::cin >> tmp_opt;
    if (std::cin.fail()) {
      std::cin.clear();
...
void ConsoleInterface::byeMsg() const { std::cout << "Goodbye!" << std::endl; }
...
void ConsoleInterface::loadGraph() {
  ...
  if (graph_ == nullptr)
    std::cout << "If not any graph loaded, you will be returned until any "
                 "valid graph is loaded."
              << std::endl;
...
void ConsoleInterface::dfs() {
  ...
  try {
    ...
    std::cout << "[SUCCESS] Depth first search completed successfully."
              << std::endl;
...
void ConsoleInterface::bfs() {
  ...
  try {
    ...
    std::cout << "[SUCCESS] Breadth first search completed successfully."
              << std::endl;
...
void ConsoleInterface::dijkstra() {
  ...
  try {
    int cost = s21::GraphAlgorithms::GetShortestPathBetweenVertices(
        *graph_, start_vertex, end_vertex);
    std::cout << "[SUCCESS] Finding shortest path between vertices "
              << start_vertex << " and " << end_vertex
...
void ConsoleInterface::floydWarshall() {
  ...
  std::cout << "[FOR FINDING SHORTEST PATH BETWEEN ALL VERTICES COMBINATIONS]"
            << std::endl;

  try {
    auto costs =
        s21::GraphAlgorithms::GetShortestPathsBetweenAllVertices(*graph_);
    std::cout << "[SUCCESS] Finding shortest path between all vertices "
...
void ConsoleInterface::prim() {
  ...
  try {
    ...
    std::cout
        << "[SUCCESS] Finding minimal spanning tree completed successfully."
        << std::endl;
...
void ConsoleInterface::antColony() {
  ...
  try {
    ...
    std::cout << "Total TSM distance: " << tsm_result.distance << "."
              << std::endl;
...
void ConsoleInterface::branchesAndBounds() {
  ...
  try {
    auto tsm_result =
        s21::GraphAlgorithms::SolveTravelingSalesmanProblemBranchesAndBounds(
            *graph_);
    std::cout << "[SUCCESS] Solving TSM completed successfully." << std::endl;
    std::cout << "Total TSM distance: " << tsm_result.distance << "."
              << std::endl;
...
void ConsoleInterface::genetic() {
  ...
  try {
    auto tsm_result =
        s21::GraphAlgorithms::SolveTravelingSalesmanProblemGenetic(*graph_);
    std::cout << "[SUCCESS] Solving TSM completed successfully." << std::endl;
    std::cout << "Total TSM distance: " << tsm_result.distance << "."
              << std::endl;
    std::cout << "Way of TSM: " << std::endl;
...
void ConsoleInterface::tsmResearch() {
  std::cout << "\n[RESEARCH BETWEEN TRAVELING SALESMAN PROBLEM ALGORITHMS]"
            << std::endl;
  std::cout << "[THERE ARE THREE DIFFERENT ALGORITHMS FOR SOLVING TRAVELING "
               "SALESMAN PROBLEM]"
            << std::endl;
  std::cout << "[ANT COLONY, GENETIC ALGORITHM AND BRANCHES AND BOUNDS]"
            << std::endl;
  std::cout << "[THE PURPOSE OF RESEARCH IS TO COMPARE THE SPEED OF EXECUTION "
               "OF THESE ALGORITHMS]"
            << std::endl;
  ...
  try {
    ...
    auto bnb_ft = std::async([N, this]() {
      auto start =
          std::chrono::high_resolution_clock().now().time_since_epoch();
      for (size_t i = 0; i < N; i++) {
        auto tsm_result = s21::GraphAlgorithms::
            SolveTravelingSalesmanProblemBranchesAndBounds(*graph_);
      }
      auto end = std::chrono::high_resolution_clock().now().time_since_epoch();
    ...
    std::cout << "Total time taken by ant colony algorithm: " << ants_ft.get()
              << " seconds." << std::endl;

    std::cout << "Total time taken by genetic algorithm: " << genetic_ft.get()
              << " seconds." << std::endl;
...
void ConsoleInterface::printAdjacencyMatrix(
